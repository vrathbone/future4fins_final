...
## Save as tif
writeRaster(sprob_combined_inverse_30, here("final_results", "scenario_5_inverse",  "sprob_combined_inverse_30_6hr_blm0.tif"), overwrite = TRUE)v
```
...
#### RUN 9: Inverse - 20% target, boundary penalty = 0, and no MPAs locked in
```{r}
## Define the problem with the cost, conservation features, targets, locked in and locked out constraints using the minimum set objective
...
#### RUN 10: Inverse - 20% target, boundary penalty = 0.000001, and no MPAs locked in
```{r}
## Define the problem with the cost, conservation features, targets, locked in and locked out constraints using the minimum set objective
prob_inverse_20_blm_nompa <- problem(cost_combined_inverse, features = features_stack_5) %>%
  add_min_set_objective() %>%
  add_relative_targets(0.2) %>%
  add_gurobi_solver(gap = 0.01, time_limit = 21600) %>% 
  add_boundary_penalty(0.000001)

## Solve problem
sprob_inverse_20_blm_nompa <- solve(prob_inverse_20_blm_nompa)

## Plot the solution to see what it looks like
plot(sprob_inverse_20_blm_nompa,  main = c("inverse- 20% Targets no MPAs and boundary penalty"))

## Save as tif
writeRaster(sprob_inverse_20_blm_nompa, here("final_results", "sensitivity", "mpas",  "sprob_combined_inverse_20_6hr_blm0.000001_nompa.tif"), overwrite = TRUE)
```

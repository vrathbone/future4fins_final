...
##1 Royal Penguin
#### BLM: .0000001, Combined, 20% Target, MPAs locked in - 20% target, 6hr time limit
```{r}
...
prob_combined_20 <- problem(cost_combined, features = features_stack_4) %>%
  ...
  add_boundary_penalties(0.0000001)
...
#Save plot
writeRaster(sprob_combined_20, here("final_results", "sprob_combined_20_6hr_blm0.0000001.tif"), overwrite = TRUE)

```

##2 Islay
#### BLM: .000001, Combined, 20% Target, MPAs locked in - 20% target, 6hr time limit
```{r}
### OBJECTIVE: Minimize combined fishing conflict while meeting 20% representation targets, locking in MPAs, and a boundary penalty of 0.0000001
...
#Save plot
writeRaster(sprob_combined_20, here("final_results", "sprob_combined_20_6hr_blm0.000001.tif"), overwrite = TRUE)

```

##3 Dugong
#### BLM: .00001, Combined, 20% Target, MPAs locked in - 20% target, 6hr time limit
```{r}
### OBJECTIVE: Minimize combined fishing conflict while meeting 20% representation targets, locking in MPAs, and a boundary penalty of 0.0000001
...
prob_combined_20 <- problem(cost_combined, features = features_stack_4) %>%
  ...
  add_locked_in_constraints(exist_mpas_4) %>%
  add_gurobi_solver(gap = 0.01, time_limit = 21600) %>% 
  add_boundary_penalties(0.00001)
...
#Save plot
writeRaster(sprob_combined_20, here("final_results", "sprob_combined_20_6hr_blm0.00001.tif"), overwrite = TRUE)

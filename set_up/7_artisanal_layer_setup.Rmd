...
art_fish[is.na(art_fish[])] <- 0.001
plot(art_fish)

art_fish <- mask(art_fish, mz_eez_buffer_sf)
plot(art_fish)

art_fish[art_fish == 0] <- 0.001

art_fish_test <- as.data.frame(art_fish)
plot(art_fish)
...
writeRaster(cost_art_fish, here('set_up/final_rasters/costs/cost_art_fish.tif'), overwrite = TRUE)
```

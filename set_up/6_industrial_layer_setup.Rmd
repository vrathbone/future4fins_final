...
This is the first of many different cost layers we added to our prioritization model. Industrial fishing is a major threat to shark and ray species largely due to bycatch, so we wanted to include it in our analysis. The industrial fishing pressure data we obtained from our clients WCS and the Mozambique government is vessel monitoring system (VMS) data for four different types of vessels: long line, gamba, shrimp, and tuna. Each data set is a separate raster that we combined in order to find total industrial fishing pressure; however, it is possible to run this analysis for each vessel type. 
...
An interactive map of the final industrial fishing pressure raster
```{r}

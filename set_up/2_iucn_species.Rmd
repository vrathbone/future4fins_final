...
Our analysis specifically focuses on sharks and rays that inhabit or migrate through the Mozambique's EEZ. From the 180 different species of shark and rays, we examined the species distribution of 27 critically endangered species using IUCN's conservation status and distributions. These distributions will be converted into rasters which will then be used in the conservation features in the `Prioritzr` package. More species can be added to this Rmd, but just need to be added to the species vector. 

#### **STEP 1: Read in Mozambique EEZ and Shapefile**
We set up a file path to our project area raster created in `1_MZ_EEZ_raster.Rmd` due to limited storage on Github. The file path will differ, but here make sure to read in the protect area raster with the cell ids `mz_rast_id` and the project area shapefile `mz_eez_sf`.
...
Here are two examples step-by-step for two species, Thresher sharks and Dusky shark, that start as sf objects to final rasters.
...
We created a function that based on the steps above (species sf --> species raster) and saves the raster in our designated folder for finalized species rasters.
```{r}
...
Here is our loop that contains the `create_rast` function. The loop will cycle through each species name and save the finalized raster in our species folder. 
```{r}
### create loop 
...
Our client suggested using refined species distributions for three of our 27 species: Pseudoginglymostoma brevicaudatum, Rhynchobatus australiae, Rhynchobatus djiddensis. Because these shapefiles are slightly different from the IUCN format, we decided to manually rasterize these distributions instead of using the loop. 
```{r}
...
Once we have all our species raster, we need them to be one raster or technically a raster stack for the `Prioritzr` conservation features. To run this, you **must** clear the environment first.
```{r}

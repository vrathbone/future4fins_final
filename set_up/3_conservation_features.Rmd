...
In this Rmarkdown, we are creating our critical habitat final rasters which include: coral, seagrass, mangroves, seamounts, and knolls. This will be added to the `Prioritzr` conservation features along with the species distributions created in `2_iucn_species.Rmd`. Other habitat shapefiles can be used following the same setup.

### **STEP 1: Read in Mozambique EEZ and Shapefile**
Grabbing the project area from the `1_MZ_EEZ_raster.Rmd` with the cell ids and the 100m resolutions. 
```{r}
## read in MZ EEZ tif
...
### **STEP 2: Create all habitat rasters**
The following code goes through how we rasterized each habitat. They all follow similar steps, but in same cases we needed to aggregate the data to be able to view the data. Each finalized habitat raster was saved in the conservation features habitat folder. All raw data was saved on local computer initially so follow personal file path to each data set. More or less habitat can be added if needed. 

#### Read in ACA data for Coral and Seagrass Shapefiles
```{r}
...
We need one raster that contains all habitat raster for `Prioritzr` so we create a raster stack of the few we created and save.
...
The conservation features for our model is both the critical habitats and species distributions, so we need to create a raster stack of the two raster stack: `species_stack.tif` & `habitat_stack.tif`. 
...
Coral Reefs: Jiwei Li, David E. Knapp, Nicholas S. Fabina, Emma V. Kennedy, Kirk Larsen, Mitchell B. Lyons, Nicholas J. Murray, Stuart R. Phinn, Chris M. Roelfsema, and Gregory P. Asner. A global coral reef probability map generated using convolutional neural networks. Coral Reefs. 2020. Advance online publication. https://doi.org/10.1007/s00338-020-02005-6

Mangroves: Giri C, Ochieng E, Tieszen LL, Zhu Z, Singh A, Loveland T, Masek J, Duke N (2011). Status and distribution of mangrove forests of the world using earth observation satellite data (version 1.3, updated by UNEP-WCMC). Global Ecology and Biogeography 20: 154-159. Paper DOI: 10.1111/j.1466-8238.2010.00584.x; Data URL: http://data.unep-wcmc.org/datasets/4

Seamounts and Knolls: Yesson C, Clark MR, Taylor M, Rogers AD (2011). The global distribution of seamounts based on 30-second bathymetry data. Deep Sea Research Part I: Oceanographic Research Papers 58: 442-453. DOI: http://dx.doi.org/10.1016/j.dsr.2011.02.004. Data DOI: http://doi.pangaea.de/10.1594/PANGAEA.757564.
